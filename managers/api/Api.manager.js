        ...
        Object.keys(this.managers).forEach(mk => {
            if (this.managers[mk][this.prop]) {
                ...
                this.managers[mk][this.prop].forEach(i => {
                    ...
                    let fnName = i;
                    if (i.includes("=")) {
                        const [methodPart, fnNamePart] = i.split('=');
                        method = methodPart.trim();
                        fnName = fnNamePart.split('/')[0].trim(); // Exclude path parameters
                    }
        ...
        let hotBolt = this.mwsExec.createBolt({
            stack: targetStack, req, res, onDone: async ({ req, res, results }) => {
                ...
                if (result.selfHandleResponse) {
                    ...
                    if (result.errors) {
                        return this.managers.responseDispatcher.dispatch(res, { ok: false, errors: result.errors, code: result?.code, message: result?.message });
                    } else if (result.error) {
                        return this.managers.responseDispatcher.dispatch(res, { ok: false, message: result.error, code: result?.code, message: result?.message });
                    } else {
                        const { status, message, data } = result;
                        return this.managers.responseDispatcher.dispatch(res, { ok: true, data: data, message, code: status });
                    }
                }
            }
        });
